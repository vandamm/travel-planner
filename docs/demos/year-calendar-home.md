...
A new root page turns the planner into a year-at-a-glance atlas. Trips occupy their real calendar dates, continue cleanly across month and year boundaries, and remain available as direct board links in the trip index.
...
```bash
npm test -- src/features/home/yearCalendar.test.ts worker/src/rooms.test.ts >/dev/null && echo '7 focused tests passed'
```
