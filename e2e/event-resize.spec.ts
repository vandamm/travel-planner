...
test('both resize edges show snapped live timing without moving overlapping neighbors', async ({
  ...
  const sortable = (title: string) =>
    column.locator('[data-testid="sortable-card"]', { hasText: title })
  ...
  await expect(preview.getByTestId('card-time')).toHaveText('09:45 · 1h 15m')
  await expect(page.getByRole('button', { name: /Resize Museum/ })).toHaveCount(0)
  await page.mouse.up()

  await expect(card('Museum').getByTestId('card-time')).toHaveText('09:45 · 1h 15m')
  await expect(card('Breakfast').getByTestId('card-time')).toHaveText('09:30 · 1h 00m')
  ...
  await expect(preview.getByTestId('card-time')).toHaveText('09:45 · 1h 30m')
  await expect(page.getByRole('button', { name: /Resize Museum/ })).toHaveCount(0)
  await page.mouse.up()

  await expect(card('Museum').getByTestId('card-time')).toHaveText('09:45 · 1h 30m')
  await expect(card('Breakfast').getByTestId('card-time')).toHaveText('09:30 · 1h 00m')
  await expect(card('Lunch').getByTestId('card-time')).toHaveText('11:00 · 1h 00m')
