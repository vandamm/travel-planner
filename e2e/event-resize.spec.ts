...
test('both resize edges show snapped live timing without moving overlapping neighbors', async ({
  ...
  await expect(preview.getByTestId('card-time')).toHaveText('09:45 – 11:00 · 1h 15m')
  ...
  await expect(preview.getByTestId('card-time')).toHaveText('09:45 – 11:15 · 1h 30m')
  await expect(page.getByRole('button', { name: /Resize Museum/ })).toHaveCount(0)
  await page.mouse.up()

  await expect(card('Museum').getByTestId('card-time')).toHaveText('09:45 – 11:15 · 1h 30m')
  await expect(card('Breakfast').getByTestId('card-time')).toHaveText('09:30 – 10:30 · 1h')
  await expect(card('Lunch').getByTestId('card-time')).toHaveText('11:00 – 12:00 · 1h')
