...
// Monday) and a first→last range-selection reducer.
//
// The handoff draws an `S M T W T F S` row, but that is the one place it assumes
// a US week: this app is European throughout (dd.MM dates, 24-hour times) and
// the home year-calendar already builds Monday-first weeks. Two calendars
// disagreeing on where a week starts is worse than diverging from the mock.
//
// Values are ISO 'yyyy-MM-dd', so a lexicographic string compare is
...
/** Which day a week starts on — Monday, matching the home year calendar. */
const WEEK_STARTS_ON = 1

/** Weekday header labels, Monday-first; the order mirrors {@link monthGrid}. */
...
export function monthGrid(year: number, month: number): GridDay[][] {
  ...
  const gridStart = startOfWeek(startOfMonth(first), { weekStartsOn: WEEK_STARTS_ON })
  const gridEnd = endOfWeek(endOfMonth(first), { weekStartsOn: WEEK_STARTS_ON })
