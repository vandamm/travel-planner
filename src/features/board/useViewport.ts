...
function getSnapshot(): Viewport {
  return selectViewport(typeof window !== 'undefined' ? window.innerWidth : DESKTOP_BREAKPOINT)
}

function getColumnsSnapshot(): number {
  return columnsThatFit(typeof window !== 'undefined' ? window.innerWidth : DESKTOP_BREAKPOINT)
}
