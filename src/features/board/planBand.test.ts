...
describe('planBandHeightPx', () => {
  it('is one hour, or the whole gap when the gap is shorter', () => {
    expect(planBandHeightPx(10 * PX_PER_HOUR)).toBe(PX_PER_HOUR)
    expect(planBandHeightPx(PX_PER_HOUR)).toBe(PX_PER_HOUR)
    // A gap shorter than an hour collapses the band to the gap.
    expect(planBandHeightPx(PX_PER_HOUR * 0.75)).toBe(PX_PER_HOUR * 0.75)
  })
})
...
describe('planBandTopPx', () => {
  it('centres the band on the pointer, snapped to quarter-hours', () => {
    ...
    const gap = 10 * PX_PER_HOUR
    // Pointer five hours down: the band centres on it, then snaps to a quarter.
    expect(planBandTopPx(5 * PX_PER_HOUR, gap)).toBe(4.5 * PX_PER_HOUR)
    // Half a quarter past a step rounds up to the next one.
    expect(planBandTopPx(PX_PER_HOUR + PX_PER_HOUR / 2 + quarter * 0.6, gap)).toBe(
      PX_PER_HOUR + quarter,
    )
  })
  ...
  it('clamps the band inside the gap at both ends', () => {
    ...
    expect(planBandTopPx(PX_PER_HOUR / 3, PX_PER_HOUR * 0.75)).toBe(0)
  })
})
...
describe('planBandTiming', () => {
  it('seeds exactly the band the user saw — one hour, wherever it sits', () => {
    ...
    expect(planBandTiming('09:00', '19:00', 2.75 * PX_PER_HOUR)).toEqual({
      startTime: '11:45',
      durationHours: 1,
    })
  })
