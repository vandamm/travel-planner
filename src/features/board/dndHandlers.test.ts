...
describe('applyCardDragEnd — untimed cards only', () => {
  it('keeps an untimed card untimed when dropped between timed cards', () => {
    const doc = freshDoc()
    addCard(doc, { dayKey: DAY1, title: 'A', startTime: '10:00' })
    const b = addCard(doc, { dayKey: DAY1, title: 'B', startTime: '16:00' }).id
    const u = addCard(doc, { dayKey: DAY1, title: 'U' }).id
    applyCardDragEnd(doc, { activeId: u, overId: b })
    expect(getCard(doc, u)?.startTime).toBeUndefined()
    expect(titles(doc, DAY1)).toEqual(['A', 'U', 'B'])
  })

  it('does not move a timed card', () => {
    const doc = freshDoc()
    const timed = addCard(doc, { dayKey: DAY1, title: 'A', startTime: '10:00' }).id
    const target = addCard(doc, { dayKey: DAY2, title: 'B' }).id
    applyCardDragEnd(doc, { activeId: timed, overId: target })
    expect(getCard(doc, timed)?.dayKey).toBe(DAY1)
    expect(titles(doc, DAY2)).toEqual(['B'])
  })
})
