...
/** The free portions of a day window after timed activities occupy their ranges. */
export function freeTimelineSlots(cards: Card[], dayStart: string, dayEnd: string): TimelineSlot[] {
  ...
  const intervals = cards
    ...
    .map((card) => ({
      ...
      end: Math.min(end, clockMinutes(card.startTime!) + resolvedDurationHours(card, dayStart, dayEnd) * 60),
    }))
  ...
  for (const interval of intervals) {
    if (interval.start > cursor) slots.push({ startTime: clockString(cursor), endTime: clockString(interval.start) })
